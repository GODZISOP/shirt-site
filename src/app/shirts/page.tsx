...
export default function ShirtsPage() {
  ...
  const shirtProducts = useMemo(() => products.filter((p) => p.category === "shirts"), [products]);
  const [activeTech, setActiveTech] = useState<Technique | "all">("all");

  const filtered = useMemo(() => {
    if (activeTech === "all") return shirtProducts;
    return shirtProducts.filter((p) => p.techniques.includes(activeTech));
  }, [shirtProducts, activeTech]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f8f9f9]">
      ...
      {/* Hero */}
      <section className="w-full bg-gradient-to-br from-[var(--color-dark-blue)] to-[var(--color-primary)] text-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm text-sm font-semibold mb-6">
              <Sparkles size={16} /> Custom Shirts & Apparel
            </div>
            <h1 className="text-3xl md:text-5xl font-extrabold font-roboto-slab tracking-tight mb-4 leading-tight">
              Custom Shirts<br />For Every Style
            </h1>
            <p className="text-blue-200 text-lg max-w-md mb-8">
              T-shirts, hoodies, sweatshirts, jackets — all customizable with embroidery, print, or laser patches.
            </p>
            <Link
          ...
          <div className="relative h-[300px] md:h-[400px]">
            <Image
              src="/Shirt/florida-dtf-prints-services.png"
              alt="Custom Shirts"
              fill
